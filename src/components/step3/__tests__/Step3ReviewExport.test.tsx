...
describe('Step3ReviewExport', () => {
  describe('EXPO-01: copy plain-text summary to clipboard', () => {
    ...
    it('calls navigator.clipboard.writeText with non-empty string when Copy Summary clicked', async () => {
      ...
      await vi.waitFor(() => {
        expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringMatching(/.+/));
      });
    });
    ...
    it('clipboard text includes current cluster data', async () => {
      ...
      await vi.waitFor(() => {
        const callArg = (navigator.clipboard.writeText as ReturnType<typeof vi.fn>).mock
          .calls[0]?.[0] as string;
  ...
  describe('REVIEW-04: iOS Safari PPTX guard', () => {
    it('shows toast instead of calling exportPptx on iOS', async () => {
      ...
      await waitFor(() => {
        expect(toast.info).toHaveBeenCalledWith(expect.stringMatching(/not supported in safari/i));
      });
