...
import { type ChartCapture, chartRefToDataUrl } from '@/lib/utils/chartCapture';
...
function headerCell(text: string) {
  return {
    text,
    options: { bold: true, fill: { color: NAVY }, color: WHITE, fontSize: 10, fontFace: FONT },
  };
}
function dataCell(text: string, rowIdx: number, bold = false) {
  return {
    text,
    options: {
      fill: { color: rowIdx % 2 === 0 ? LIGHT_GRAY : WHITE },
      fontSize: 10,
      fontFace: FONT,
      bold,
    },
...
function addFooter(
  slide: {
    addText: (
      t: string | Array<{ text: string; options?: Record<string, unknown> }>,
  ...
  slide.addText(
    [
      ...
      {
        text: `  |  ${date}  |  Slide ${_slideNum}`,
        options: { color: GRAY, fontSize: 7, fontFace: FONT },
      },
    ],
...
export async function exportPptx(
  ...
  pptx.defineSlideMaster({
    ...
    objects: [
      {
        placeholder: {
          options: { name: 'title', type: 'title', x: 0.5, y: 2.2, w: 12, h: 1.2 },
          text: '',
        },
      },
      {
        placeholder: {
          options: { name: 'body', type: 'body', x: 0.5, y: 3.5, w: 12, h: 0.6 },
          text: '',
        },
      },
    ],
  ...
  pptx.defineSlideMaster({
    ...
    objects: [
      ...
      {
        placeholder: {
          options: { name: 'title', type: 'title', x: 0.65, y: 0.2, w: 11.85, h: 0.6 },
          text: '',
        },
      },
    ],
  ...
  summarySlide.addTable([summaryHeaderRow, ...summaryDataRows], {
    x: 0.5,
    y: 2.2,
    w: 12,
  ...
  const compMetrics: CompMetric[] = [
    ...
    },
    {
      ...
      scenarioValues: scenarios.map((s) =>
        s.haReserveCount === 0 ? 'None' : `N+${s.haReserveCount}`,
      ),
    },
    {
      label: 'Avg vCPU/VM',
  ...
  comparisonSlide.addTable([compHeaderRow, ...compDataRows], {
    x: 0.5,
    y: 1.2,
    w: 12,
  ...
  scenarios.forEach((scenario, i) => {
    ...
    if (charts?.minnodes && bd) {
      ...
      mnSlide.addText('The binding constraint determines the minimum cluster size.', {
        ...SUBTITLE_OPTS,
      });
