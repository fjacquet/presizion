...
import { type ChartCapture, chartRefToDataUrl } from './chartCapture';
...
export async function exportPdf(
  ...
  function addPageFooter(): void {
    ...
    doc.text(`Presizion  |  ${dateStr}  |  Page ${pageNum}`, PAGE_W / 2, PAGE_H - 6, {
      align: 'center',
    });
  ...
  if (bestResult) {
    ...
    drawKpi(
      MARGIN + colW * 1.5,
      String(bestResult.finalCount),
      `${scenarios[0]?.name ?? 'Target'} Servers`,
    );
    y = kpiY;
    drawKpi(
      MARGIN + colW * 2.5,
      `${bestResult.cpuUtilizationPercent.toFixed(0)}%`,
      'CPU Utilization',
    );
    y = kpiY;
    drawKpi(
      MARGIN + colW * 3.5,
      `${bestResult.ramUtilizationPercent.toFixed(0)}%`,
      'RAM Utilization',
    );
    y = kpiY + 18;
  }
  ...
  const comparisonBody: string[][] = [
    ...
    ['Limiting Resource', 'N/A', ...results.map((r) => RESOURCE_LABELS[r.limitingResource])],
    [
      'vCPU:pCore Ratio',
      asIsRatio,
      ...results.map((r) => `${r.achievedVcpuToPCoreRatio.toFixed(1)}:1`),
    ],
    ['VMs/Server', asIsVmsPerServer, ...results.map((r) => r.vmsPerServer.toFixed(1))],
    ['Avg vCPU/VM', avgVcpuPerVm, ...scenarios.map(() => avgVcpuPerVm)],
    ['Avg RAM/VM (GiB)', avgRamPerVm, ...scenarios.map((s) => s.ramPerVmGb.toFixed(1))],
    ['Avg Disk/VM (GiB)', avgDiskPerVm, ...scenarios.map((s) => s.diskPerVmGb.toFixed(1))],
    ['Safety %', 'N/A', ...scenarios.map((s) => `${s.safetyPercent}%`)],
    [
      'CPU Util %',
  ...
  const assumptionsBody: string[][] = [
    ['vCPU:pCore Ratio', ...scenarios.map((s) => `${s.targetVcpuToPCoreRatio.toFixed(1)}:1`)],
    ['Safety %', ...scenarios.map((s) => `${s.safetyPercent}%`)],
    [
      'HA Reserve',
      ...scenarios.map((s) => {
    ...
    ['RAM / VM (GB)', ...scenarios.map((s) => s.ramPerVmGb.toFixed(1))],
    ['Disk / VM (GB)', ...scenarios.map((s) => s.diskPerVmGb.toFixed(1))],
  ];
  ...
  if (hasVsan) {
    ...
    const vsanBody: string[][] = [
      [
        'FTT Policy',
        ...scenarios.map((s) => (s.vsanFttPolicy ? FTT_POLICY_MAP[s.vsanFttPolicy].label : 'N/A')),
      ],
      [
        'Compression Factor',
        ...scenarios.map((s) =>
      ...
      ['VM Swap', ...scenarios.map((s) => (s.vsanVmSwapEnabled ? 'Enabled' : 'Disabled (sparse)'))],
    ];
  ...
  const hasGrowth = scenarios.some((s) => s.growthPercent !== undefined && s.growthPercent > 0);
  if (hasGrowth) {
    ensureSpace(40);
    ...
    const growthBody: string[][] = [
      ['Growth %', ...scenarios.map((s) => `${s.growthPercent ?? 0}%`)],
    ];
  ...
  const serverConfigBody: string[][] = [
    ['Sockets / Server', ...scenarios.map((s) => String(s.socketsPerServer))],
    ['Cores / Socket', ...scenarios.map((s) => String(s.coresPerSocket))],
    [
      'Total Cores / Server',
      ...scenarios.map((s) => String(s.socketsPerServer * s.coresPerSocket)),
    ],
    ['RAM / Server (GB)', ...scenarios.map((s) => s.ramPerServerGb.toLocaleString())],
    ['Disk / Server (GB)', ...scenarios.map((s) => s.diskPerServerGb.toLocaleString())],
  ];
