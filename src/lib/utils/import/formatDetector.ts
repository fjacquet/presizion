...
async function detectZip(buffer: ArrayBuffer): Promise<DetectionResult> {
  ...
  for (const entry of xlsxEntries) {
    ...
    const score = wb.SheetNames.includes('vInfo')
      ? 3 // RVTools
      : wb.SheetNames.includes('ESX Hosts')
        ? 2 // LiveOptics VMWARE
        : wb.SheetNames.includes('VMs')
          ? 1 // LiveOptics GENERAL
          : 0; // AIR or unknown
    if (score > bestScore) {
      bestScore = score;
      bestBuffer = buf;
  ...
  if (!bestBuffer || bestScore === 0) {
    throw new ImportError(
      'No suitable xlsx found in zip. Expected RVTools (vInfo sheet) or LiveOptics (VMs or ESX Hosts sheet).',
    );
  }
