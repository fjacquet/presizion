...
export function aggregateScopes(rawByScope: RawByScopeMap, selectedKeys: string[]): ScopeData {
  ...
  for (const scope of selected) {
    ...
    if (scope.ramPerServerGb !== undefined) {
      ramPerServerEntries.push({
        ram: scope.ramPerServerGb,
        hosts: scope.existingServerCount ?? 1,
      });
    }
  ...
  if (ramPerServerEntries.length > 0) {
    ...
    if (uniqueRam.size > 1) {
      allWarnings.push(
        'Heterogeneous RAM/server detected across clusters -- using host-count-weighted average.',
      );
    }
  }
