...
function computeVhostEsxFields(
  hosts: VInfoRow[],
...
export async function parseRvtools(
  ...
  for (const [scopeKey, scope] of rawByScope.entries()) {
      ...
      dcSet && dcSet.size === 2
        ? new Map(
            Array.from(dcSet).map((dc) => [
              dc,
              rawByScope.get(`${dc}||${clusterName}`)?.existingServerCount ?? 0,
            ]),
          )
        : undefined;
    ...
    const analysis = analyzeStretchCluster({
      ...
      ...(vsanEntry &&
        vsanEntry.faultDomains.size > 0 && { faultDomainCount: vsanEntry.faultDomains.size }),
