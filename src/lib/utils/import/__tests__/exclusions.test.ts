...
import { aggregateVmRows, applyExclusions, compileNamePattern, isExcluded } from '../exclusions';
...
describe('applyExclusions', () => {
  ...
  it('returns input unchanged and zero stats when rules are empty', () => {
    ...
    expect(stats.excludedByRule).toEqual({
      namePattern: 0,
      exactNames: 0,
      powerState: 0,
      manual: 0,
  ...
  it('attributes exclusions to the first matching rule in order', () => {
    const input = new Map<string, VmRow[]>([['s1', rows(['test-a', 'test-b'])]]);
    const rules = {
      ...EMPTY_RULES,
      namePattern: 'test-*',
  ...
  it('counts exact-name exclusions separately from name pattern', () => {
    const input = new Map<string, VmRow[]>([['s1', rows(['lab-vm', 'test-a'])]]);
