...
type ScopeEntry = Omit<
  ClusterImportResult,
  'sourceFormat' | 'detectedScopes' | 'scopeLabels' | 'rawByScope'
>;
...
describe('aggregateScopes', () => {
  ...
  it('returns zeroed result when selectedKeys is empty', () => {
    const map = new Map<string, ScopeEntry>([['CL-A', makeScope({ totalVcpus: 10, vmCount: 2 })]]);
  ...
  it('socketsPerServer and coresPerSocket use representative from first scope', () => {
    const map = new Map<string, ScopeEntry>([
      [
        'CL-A',
        makeScope({ vmCount: 2, socketsPerServer: 2, coresPerSocket: 8, existingServerCount: 4 }),
      ],
      [
        'CL-B',
        makeScope({ vmCount: 3, socketsPerServer: 4, coresPerSocket: 12, existingServerCount: 6 }),
      ],
    ]);
    const result = aggregateScopes(map, ['CL-A', 'CL-B']);
  ...
  it('stretched flag on any selected scope poisons the aggregate', () => {
    const map = new Map<string, ScopeEntry>([
      [
        'CL-A',
        makeScope({ vmCount: 1, isStretchCluster: true, stretchSignals: ['vSAN says stretched'] }),
      ],
