...
beforeEach(() => {
  vi.mocked(XLSX.read).mockReturnValue(
    rvtoolsWorkbook() as unknown as ReturnType<typeof XLSX.read>,
  );
});
...
describe('formatDetector', () => {
  ...
  it('detects LiveOptics xlsx by VMs sheet name', async () => {
    vi.mocked(XLSX.read).mockReturnValue(
      liveopticWorkbook() as unknown as ReturnType<typeof XLSX.read>,
    );
    const result = await detectFormat(new ArrayBuffer(0), 'export.xlsx');
  ...
  it('throws ImportError for xlsx with unknown sheet names', async () => {
    vi.mocked(XLSX.read).mockReturnValue(
      unknownWorkbook() as unknown as ReturnType<typeof XLSX.read>,
    );
