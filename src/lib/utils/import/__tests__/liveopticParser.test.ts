...
const MOCK_ROWS = [
  ...
  },
  {
    'VM Name': 'tmpl',
    'Virtual CPU': 2,
    'Provisioned Memory (MiB)': 4096,
    'Virtual Disk Size (MiB)': 10240,
    Template: true,
  },
];
...
describe('liveopticParser', () => {
  ...
  describe('ESX Hosts sheet — cpuModel and cpuFrequencyGhz extraction', () => {
    it('extracts cpuModel from CPU Model column of first ESX host', async () => {
      ...
      vi.mocked(XLSX.utils.sheet_to_json).mockImplementation((sheet) => {
        if (sheet === HOSTS_SHEET)
          return [
            {
              'Host Name': 'esxi-01',
              'CPU Sockets': 2,
              'CPU Cores': 48,
              'Memory (KiB)': 536870912,
              'CPU Model': 'Intel Xeon Gold 6526Y',
              'CPU Speed (MHz)': 2400,
            },
          ];
        return MOCK_ROWS;
  ...
  describe('per-scope ESX host config from ESX Hosts sheet', () => {
    ...
    function setupMultiClusterWithHosts(opts?: {
      hostsHaveCluster?: boolean;
      vmHostCol?: boolean;
      perfSheet?: boolean;
    }) {
      const vmRows = [
        {
          ...
          Template: false,
          Cluster: 'CL-A',
          ...(opts?.vmHostCol ? { 'ESX Host': 'esxi-01' } : {}),
        },
        {
          'VM Name': 'db-01',
          'Virtual CPU': 8,
          ...
          Template: false,
          Cluster: 'CL-A',
          ...(opts?.vmHostCol ? { 'ESX Host': 'esxi-02' } : {}),
        },
        {
          'VM Name': 'app-01',
          'Virtual CPU': 2,
          ...
          Template: false,
          Cluster: 'CL-B',
          ...(opts?.vmHostCol ? { 'ESX Host': 'esxi-03' } : {}),
        },
      ];
        ...
        ? [
            {
              ...
              'CPU Sockets': 2,
              'CPU Cores': 48,
              'Memory (KiB)': 268435456,
              'CPU Model': 'Xeon Gold A',
              'CPU Speed (MHz)': 2400,
              Cluster: 'CL-A',
            },
            {
    ...
    it('no cluster column on ESX Hosts and no host column on VMs -> ESX data on __all__ with warning', async () => {
      ...
      expect(result.warnings).toContain(
        'Host-to-cluster mapping unavailable; ESX host config applied globally.',
      );
    });
