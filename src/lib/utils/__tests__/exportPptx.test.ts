...
describe('exportPptx', () => {
  ...
  it('defines CONTENT_SLIDE master with navy accent strip rect', async () => {
    const { exportPptx } = await import('../exportPptx');
    await exportPptx(cluster, [scenario], [result], [breakdown], {});
    const dsmCalls = mockDefineSlideMaster.mock.calls as Array<
      [{ title: string; objects: Array<Record<string, unknown>> }]
    >;
  ...
  it('uses navy fill in table header cells (not blue)', async () => {
    ...
    const tableCalls = mockAddTable.mock.calls as Array<
      [Array<Array<{ text: string; options?: { fill?: { color: string } } }>>, unknown]
    >;
  ...
  it('includes server config rows (e.g. "Sockets / Server") in the comparison table', async () => {
    ...
    const textCalls = mockAddText.mock.calls as Array<
