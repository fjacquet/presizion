/**
 * Application configuration — external links and settings.
 *
 * Change these values to point to different instances.
 */

/** Store-Predict storage calculator */
export const STORE_PREDICT_URL = 'https://store.srv1023035.hstgr.cloud/';

/** SPEC search GitHub Pages API base URL */
export const SPEC_SEARCH_API_URL = 'https://fjacquet.github.io/spec-search';

/** SPEC search GitHub Pages web URL */
export const SPEC_SEARCH_WEB_URL = 'https://fjacquet.github.io/spec-search';

/** SPEC search CPU2017 JSON data root — spec-search nests per-benchmark since jbb2015 was added. */
export const SPEC_SEARCH_CPU2017_DATA_URL = `${SPEC_SEARCH_API_URL}/data/cpu2017`;
